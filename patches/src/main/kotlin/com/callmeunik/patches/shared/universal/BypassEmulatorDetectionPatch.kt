...
/**
 * Bypass Emulator Detection — fixed (no nop, null-safe).
 */
@Suppress("unused")
val bypassEmulatorDetectionPatch = bytecodePatch(
    ...
    execute {
        ...
        val emulatorStrings = listOf(
            ...
            "android sdk built for x86", "genymotion", "bluestacks",
            "nox", "memu", "ldplayer", "andy", "ttvm", "vbox",
            "virtualbox", "qemu", "goldfish", "ranchu", "sdk_gphone",
            "sdk_google", "droid4x", "tiantian", "windroye", "microvirt",
        )

        fun String.hasEmulatorHint() =
            emulatorHints.any { contains(it, ignoreCase = true) }

        classDefForEach { classDef ->
            mutableClassDefBy(classDef).methods.forEach { method ->
                // FIX: null implementation
                if (method.instructionsOrNull == null) return@forEach

                val name = method.name
                val instructions = method.instructionsOrNull!!.toList()

                // Method name looks like emulator check → force safe result
                ...
                instructions.forEachIndexed { index, instruction ->
                    ...
                    if (!ref.name.hasEmulatorHint() &&
                        !ref.definingClass.hasEmulatorHint()
                    ) {
                        return@forEachIndexed
                    }
